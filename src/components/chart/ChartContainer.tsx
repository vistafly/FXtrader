...
export function ChartContainer({
  ...
  useEffect(() => {
    ...
    // Seed with whatever bars the engine already has and live-subscribe.
    const seedAndSubscribe = () => {
      const engine = useReplayStore.getState().engine;
      if (!engine) return () => {};
      handle.setData(engine.getVisibleBars());
    ...
    // If the store's engine reference changes (e.g. loadInstrument is called
    // after mount), drop the old subscription and re-seed.
    const unsubStore = useReplayStore.subscribe((state, prev) => {
      if (state.engine !== prev.engine) {
        unsubEngine();
  ...
  }, [providerFactory, symbol]);

  return (
    <div className={className ?? "relative h-full w-full"}>
      <div ref={containerRef} className="h-full w-full" />
      {symbol && <PositionDragOverlay handleRef={handleRef} symbol={symbol} />}
...
function CrosshairTooltip({
  ...
  return (
    <div
      className="pointer-events-none absolute left-3 top-3 z-10 rounded-md border border-border bg-card/90 px-3 py-2 font-mono text-xs leading-tight backdrop-blur-sm"
      role="status"
      aria-live="polite"
    >
