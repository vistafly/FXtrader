...
- **Per-pane scroll-position persistence** — attempted in v2.2.6c (commit 3b51687…794adff range), reverted. The naive implementation raced against `LightweightChartProvider.subscribeVisibleRange`'s immediate-fire emission, which clobbered the saved range on disk before restore could run; a follow-up fix introduced a scrollToPosition(0) regression on the Today button. The fit-to-data default on reload is acceptable for short replay sessions. Revisit if friend-test feedback flags it as missing — likely needs a decoupled persistence pipeline (debounced explicit save on user pan-end, not subscription-fire) and a closure snapshot pattern.
- **Server-side reducer for D3 liquidation rule re-check** — v2.3 ships the substrate (`attemptDiscrepancies` Convex table + `recordDiscrepancy` mutation) but not the actual server-side rule re-check. Implementing it requires porting `AttemptReducer` and `MatchingEngine.computePnl` into the Convex runtime (which can't import from `src/`) and wiring an `appendEvent` hook that replays the log so far through the server-side reducer to derive its own balance/equity, then compares against the client's reported state on `liquidation` events. Substantial scope: ~200 lines of inlined reducer + computePnl + computeCommission, plus per-event replay overhead at append time, plus tests asserting server-side replay ≡ client-side reducer. Belongs in the v2.x anti-cheat phase alongside per-bar equity history and full event-log verification. Friends-only scale acceptable without it; revisit when opening to public lobbies.
